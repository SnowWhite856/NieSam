...
public class EquipBtnScript : MonoBehaviour
{
    ...
    public void EquipItemClick()
    {
        ...
        var wap = FindFirstObjectByType<EqScipt>();
        switch (toEquip.tag)
        {
            case "wapon":
                if (wap.currentWapon != null)
                {
                    var pastWapon = wap.currentWapon;
                    wap.item.Add(pastWapon);

                    //Debug.Log(pastWapon.name);
                    FindFirstObjectByType<PlayerStats>().TakeOffItem(pastWapon, "wapon");
                }


                wap.currentWapon = (WaponsClass)toEquip;
                wap.item.Remove(toEquip);
